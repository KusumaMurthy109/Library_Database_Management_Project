...
CREATE TABLE Library_Transaction (
  ...
  CHECK (return_date >= checked_out_date OR return_date = NULL),
...
CREATE TABLE Book (
    ...
    PRIMARY KEY (item_id),
    FOREIGN KEY (item_id) REFERENCES Library_Item (item_id),
    UNIQUE (ISBN)
);
...
CREATE TABLE Member_Account (
	...
	recommended_items VARCHAR(30) NOT NULL,
...
CHECK (waitlist_status IN ('on_hold', 'available_for_checkout'))
);
...
CREATE TABLE Generate (
	...
	staff_id INT,
	PRIMARY KEY (report_id),
	FOREIGN KEY (report_id) REFERENCES Library_Report (report_id),
	FOREIGN KEY (staff_id) REFERENCES Staff (staff_id)
);
...
CREATE TABLE Rating (
	...
	FOREIGN KEY (member_id) REFERENCES Library_Item (item_id),
	FOREIGN KEY (item_id) REFERENCES Library_Member (member_id),
	CHECK (stars_given >= 0 AND stars_given <= 5) 
);
